...
import { getErrorList } from '../utils/booksHelpers'
...
async function submit(form) {
  ...
  try {
    ...
    router.push({
      name: 'books.index',
      query: {
        success: response?.message || 'Book berhasil ditambahkan.',
      },
    })
  } catch (error) {
...
<template>
  <div>
    <div class="page-header">
      <h2 class="h4 mb-0 page-title">Create Book</h2>
    ...
    <BookForm :loading="isSaving" :require-image="true" submit-label="Simpan" @submit="submit" @cancel="cancel" />
  </div>
</template>
